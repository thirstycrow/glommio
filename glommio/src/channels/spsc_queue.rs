...
impl<T> Buffer<T> {
    ...
    fn try_pop(&self) -> Option<T> {
        ...
        let cell = self.buffer_storage[current_head % self.ccache.capacity].as_ptr();
        let v = unsafe { ptr::read(cell) };
    ...
    fn try_push(&self, v: T) -> Option<T> {
        ...
        let cell = self.buffer_storage[current_tail % self.pcache.capacity].as_ptr();
        unsafe { ptr::write(cell, v) }
...
fn allocate_buffer<T>(capacity: usize) -> Arc<Vec<Cell<T>>> {
